...
.hc_plot_deg_dist_driver <- function() {
  for (x in base::seq_along(hcobject[["layers"]])) {
    if (!hcobject[["cutoff_vec"]][x] %in% hcobject[["layer_specific_outputs"]][[base::paste0("set", x)]][["part1"]][["cutoff_stats"]][["cutoff"]]) {
      ...
      cutoff_vec <- hcobject[["cutoff_vec"]]
      cutoff_vec[x] <- tmp_vec[base::which.min(base::abs(tmp_vec - hcobject[["cutoff_vec"]][x]))]
      .hc_set_bridge_hcobject_slot("cutoff_vec", cutoff_vec)
    }
    ...
    dd_plot_calculated_optimal <- ggplot2::ggplot(stats_calculated_optimal_cutoff, ggplot2::aes(x = base::log(degree), y = base::log(Probs))) +
      ...
      ggplot2::labs(
        ...
        subtitle = base::paste0(
          "Cut-off: ", hcobject[["cutoff_vec"]][x], "; R^2: ", base::round(stats[1], 3), "; no. edges: ",
    ...
    .hc_display_object(dd_plot_calculated_optimal)
