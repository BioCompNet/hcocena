...
.hc_change_grouping_parameter_driver <- function(group_by, col_order = NULL, cluster_columns = FALSE, row_order = NULL, cluster_rows = TRUE) {
  ...
  on.exit(
    {
      .hc_set_bridge_hcobject_slot(c("global_settings", "control"), old_control)
      .hc_set_bridge_hcobject_slot(c("global_settings", "voi"), old_voi)
      .hc_set_bridge_hcobject_slot(c("integrated_output", "GFC_all_layers"), old_gfc_all_layers)
      .hc_set_bridge_hcobject_slot("data", old_data)
      .hc_set_bridge_hcobject_slot("layer_specific_outputs", old_layer_specific)
      if (sat_outputs_present) {
        .hc_set_bridge_hcobject_slot(c("satellite_outputs", "column_annos_categorical"), old_col_annos_categorical)
        .hc_set_bridge_hcobject_slot(c("satellite_outputs", "column_annos_numerical"), old_col_annos_numerical)
      }
    },
    add = TRUE
  )
  ...
  for (i in base::seq_along(hcobject[["layers"]])) {
    ...
    if (!hcobject[["global_settings"]][["control"]] == "none") {
      ...
      .hc_set_bridge_hcobject_slot(c("global_settings", "control"), "none")
      message("Regrouped GFCs will be calculated without reference to controls.")
    }
  ...
  .hc_set_bridge_hcobject_slot(c("integrated_output", "GFC_all_layers"), sep_GFCs)
  .hc_set_bridge_hcobject_slot("data", tmp_data_full)
  .hc_set_bridge_hcobject_slot("layer_specific_outputs", tmp_layer_specific)
  .hc_set_bridge_hcobject_slot(c("global_settings", "voi"), "regrouped")
