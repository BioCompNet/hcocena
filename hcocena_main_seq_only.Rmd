...
```{r GO profiling, fig.width = 9, fig.height = 9, message = F, warning = F}
...
```{r optional module celltype annotation, fig.width = 12, fig.height = 8, message = TRUE, warning = FALSE}
...
## OPTIONAL: 3.4 Upstream regulator and pathway inference (DoRothEA + PROGENy)
  ...
  - `consistent_terms` controls how per-comparison pages are rendered:
    - `FALSE` (default): only local (current-condition) selected activities, no significance stars.
    - `TRUE`: uses a global all-condition term axis for comparability, but still plots values from the currently shown condition only; stars (`*`) mark terms significant in that shown condition.
...
```{r upstream inference, fig.width = 12, fig.height = 8, message = TRUE, warning = FALSE}
...
```{r module knowledge network, fig.width = 15, fig.height = 9, message = FALSE, warning = FALSE}
