...
#' `hc_plot_cluster_heatmap(include_module_significance = TRUE)`.
#'
...
module_condition_significance <- function(set = "all",
  ...
  .hc_set_bridge_hcobject_slot(c("satellite_outputs", slot_name), slot_out)
  ...
  for (cl in base::rownames(mat)) {
    ...
    for (ri in base::seq_len(base::nrow(pv))) {
      for (ci in base::seq_len(base::ncol(pv))) {
        qv <- .hc_first_numeric_value(pv[ri, ci])
...
.hc_mc_run_limma <- function(module_mat,
  ...
  summary <- if (base::nrow(pairwise) > 0) {
    ...
    summary_rows <- base::lapply(base::names(split_pw), function(cl) {
      sub <- split_pw[[cl]]
      qv <- .hc_as_numeric_safely(sub$adj.P.Val)
      ...
      base::data.frame(
        ...
        logFC = .hc_first_numeric_value(sub$logFC[idx]),
        p = .hc_first_numeric_value(sub$P.Value[idx]),
        p_adj = .hc_first_numeric_value(sub$adj.P.Val[idx]),
        t = .hc_first_numeric_value(sub$t[idx]),
        B = .hc_first_numeric_value(sub$B[idx]),
        stringsAsFactors = FALSE
      )
    })
  ...
  rows <- base::lapply(base::rownames(mat), function(cl) {
    ...
    if (isTRUE(include_time) && "time" %in% base::colnames(df)) {
      ...
      if (use_time) {
        if (base::all(grepl("^[-+]?[0-9]*\\.?[0-9]+$", base::as.character(df$time[has_time])))) {
          df$time <- .hc_as_numeric_safely(df$time)
        } else {
    ...
    if (!base::is.null(a)) {
      ...
      if (base::length(p_col) > 0 && "cond" %in% base::rownames(a)) {
        p <- .hc_first_numeric_value(a["cond", p_col[[1]]])
      }
    }
