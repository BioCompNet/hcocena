.hc_select_enrichment_rows <- function(significant_rows,
                                       top,
                                       cluster_levels = NULL,
                                       consistent_terms = TRUE) {
  if (base::is.null(cluster_levels) || base::length(cluster_levels) == 0) {
    cluster_levels <- base::names(significant_rows)
  }
  cluster_levels <- base::as.character(cluster_levels)
  ...
  if (!isTRUE(consistent_terms)) {
    for (cl in cluster_levels) {
      cl_rows <- significant_rows[[cl]]
      if (base::is.null(cl_rows) || base::nrow(cl_rows) == 0) {
        next
      }
      pick_n <- base::min(top, base::nrow(cl_rows))
      tmp_selected <- cl_rows[base::seq_len(pick_n), , drop = FALSE]
      tmp_selected$rank <- base::seq_len(base::nrow(tmp_selected))
      selected_rows[[cl]] <- tmp_selected
      top_enr[[cl]] <- base::as.character(tmp_selected$Description)
    }
    return(list(
  ...
  selected_terms <- base::character(0)
  for (cl in cluster_levels) {
    cl_rows <- significant_rows[[cl]]
    if (base::is.null(cl_rows) || base::nrow(cl_rows) == 0) {
      next
    ...
    pick_n <- base::min(top, base::nrow(cl_rows))
    selected_terms <- base::c(
      selected_terms,
      base::as.character(cl_rows$Description[base::seq_len(pick_n)])
    )
  }
  selected_terms <- base::unique(selected_terms)

  if (base::length(selected_terms) == 0) {
    return(list(
      selected_rows = selected_rows,
      top_enr = top_enr
    ))
  ...
  for (cl in cluster_levels) {
    ...
    tmp_selected <- cl_rows[
      base::as.character(cl_rows$Description) %in% selected_terms, ,
      drop = FALSE
    ]
    if (base::nrow(tmp_selected) == 0) {
      next
    }
    selected_rows[[cl]] <- tmp_selected
    top_enr[[cl]] <- base::as.character(tmp_selected$Description)
...
functional_enrichment <- function(gene_sets = "Hallmark",
  ...
  if (!base::is.logical(consistent_terms) ||
    base::length(consistent_terms) != 1 ||
    base::is.na(consistent_terms)) {
  ...
  if (base::is.null(hcobject[["satellite_outputs"]][["enrichments"]]) ||
    ...
    .hc_set_bridge_hcobject_slot(c("satellite_outputs", "enrichments"), list())
  } else {
    .hc_set_bridge_hcobject_slot(
      c("satellite_outputs", "enrichments"),
      as.list(hcobject[["satellite_outputs"]][["enrichments"]])
    )
  }
  ...
  build_enrichment_export <- function(res_list,
    ...
    top_n <- .hc_first_numeric_value(.hc_as_integer_safely(top_n))
  ...
  for (i in gene_sets) {
    ...
    if (i_title %in% base::names(supplementary_data)) {
      ...
      selection_info <- .hc_select_enrichment_rows(
      ...
      top_enr <- selection_info$top_enr
      ...
      for (cl in plot_cluster_order) {
        cl_res <- selected_rows[[cl]]
        if (base::is.null(cl_res) || base::nrow(cl_res) == 0) {
          next
        }
        cl_res <- cl_res[, c("Description", "qvalue"), drop = FALSE]
      ...
      draw_with_body_borders <- function(ht_obj,
        ...
        if (!base::is.null(panel_title) &&
          ...
          panel_title_slice <- .hc_first_numeric_value(.hc_as_integer_safely(panel_title_slice))
      ...
      warning("invalid database: ", i, call. = FALSE)
    }
  }
  ...
  .hc_set_bridge_hcobject_slot(c("satellite_outputs", "enrichments", "top_all_dbs"), NULL)
  .hc_set_bridge_hcobject_slot(c("satellite_outputs", "enrichments", "top_all_dbs_mixed"), NULL)
  ...
  if (base::nrow(combined_selected_summary) > 0) {
    ...
    if (base::nrow(combined_plot_df) > 0) {
      ...
      combined_plot_df$rank <- .hc_as_numeric_safely(combined_plot_df$rank)
      combined_plot_df$qvalue <- .hc_as_numeric_safely(combined_plot_df$qvalue)
      ...
      draw_with_body_borders_all <- function(ht_obj,
        ...
        slices_use <- .hc_as_integer_safely(slices_use)
      ...
      if (identical(store_panel_objects, "always")) {
        .hc_set_bridge_hcobject_slot(c("satellite_outputs", "enrichments", "top_all_dbs"), list(
      ...
      if (identical(store_panel_objects, "always")) {
        .hc_set_bridge_hcobject_slot(c("satellite_outputs", "enrichments", "top_all_dbs_mixed"), list(
  ...
  .hc_set_bridge_hcobject_slot(c("satellite_outputs", "enrichments", "all_enrichments_all_dbs"), combined_all_summary)
  .hc_set_bridge_hcobject_slot(c("satellite_outputs", "enrichments", "selected_enrichments_all_dbs"), combined_selected_summary)
  .hc_set_bridge_hcobject_slot(c("satellite_outputs", "enrichments", "significant_enrichments_all_dbs"), combined_significant_summary)
