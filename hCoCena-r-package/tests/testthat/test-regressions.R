...
test_that("regression: large result stores are no longer mirrored across legacy slots", {
  ...
  network_plot_src <- paste(deparse(get("plot_integrated_network", asNamespace("hcocena"))), collapse = "\n")
  gfc_network_src <- paste(deparse(get("plot_GFC_network", asNamespace("hcocena"))), collapse = "\n")
