...
algo_alluvial <- function() {
  ...
  for (a in cluster_algo_list[!cluster_algo_list == hcobject[["global_settings"]][["chosen_clustering_algo"]]]) {
    ...
    a2a <- base::merge(current_algo, partition_df, by = "gene", all = TRUE)
    ...
    my_color <- base::paste0(
      "d3.scaleOrdinal() .domain([", base::paste0(base::paste0('"', base::as.character(nodes$name), collapse = '", '), '"', collapse = '"'),
    ...
    p <- tryCatch(
      ...
      error = function(e) {
        warning("Could not render Sankey for `", a, "`: ", base::conditionMessage(e), call. = FALSE)
        NULL
      }
    )
