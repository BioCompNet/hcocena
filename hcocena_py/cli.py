...
def prepare_score_table(scores: Sequence[Dict[str, Any]]) -> Tuple[List[str], List[List[str]]]:
    """Return a header/rows pair covering every sample column in ``scores``."""

    if not scores:
        return [], []

    fixed_columns = ["dataset", "cluster"]
    sample_columns = sorted({key for row in scores for key in row if key not in fixed_columns})
    header = fixed_columns + sample_columns
    rows = [[str(row.get(column, "")) for column in header] for row in scores]
    return header, rows
...
def main(argv: list[str] | None = None) -> None:
    ...
    with (output_dir / "cluster_scores.tsv").open("w", encoding="utf-8") as handle:
        header, rows = prepare_score_table(scores)
        if header:
