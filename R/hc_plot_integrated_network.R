...
.hc_plot_integrated_network_driver <- function(layout = NULL,
  ...
  if (base::length(igraph::V(network)$name[!igraph::V(network)$name %in% gene_to_cluster$name]) > 0) {
    gene_to_cluster <- base::rbind(gene_to_cluster, base::data.frame(
      name = igraph::V(network)$name[!igraph::V(network)$name %in% gene_to_cluster$name],
      color = "white"
    ))
    gene_to_cluster$name <- base::as.character(gene_to_cluster$name)
    gene_to_cluster$color <- base::as.character(gene_to_cluster$color)
  ...
  if (base::is.null(layout)) {
    if (layout_algorithm == "cytoscape") {
      ...
      l <- .hc_with_seed(123L, switch(layout_algorithm,
  ...
  if (!base::is.null(gene_labels)) {
    ...
    new_edges <- base::matrix(
      base::c(
        base::match(new_genes_df$label, igraph::get.vertex.attribute(network2)$name),
    ...
    igraph::plot.igraph(network2,
      vertex.size = 3, vertex.label = new_labels, vertex.label.cex = 0.75,
