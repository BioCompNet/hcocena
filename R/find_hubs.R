...
find_hubs <- function(clusters = c("all"),
  ...
  for(col in base::colnames(hubs_df)){
    if(base::file.exists(base::paste0(hcobject[["working_directory"]][["dir_output"]], hcobject[["global_settings"]][["save_folder"]], "/Hub_genes.xlsx"))){
      ...
      openxlsx::saveWorkbook(wb,base::paste0(hcobject[["working_directory"]][["dir_output"]], hcobject[["global_settings"]][["save_folder"]], "/Hub_genes.xlsx"),overwrite = TRUE)
    }else{
  ...
  for(col in base::colnames(hubs_df)){
    if(!all(dplyr::pull(hubs_df, col) == " ")){
      visualize_gene_expression(genes = dplyr::pull(hubs_df, col), 
...
.hc_find_hubs_legacy_driver <- find_hubs
