---
title: "hCoCena single-layer workflow"
...
# 0 Introduction
  This single-layer workflow keeps the familiar `main` / `satellite` structure
while using the current object-based hCoCena API. It can be combined with
the companion `hcocena_satellite.Rmd` on this branch for optional downstream
plots and helper utilities.
...
## 1.1 Load hCoCena package
  ...
  Note: hCoCena no longer depends on `MCDA` or `propr`.
...
# 7700 and 7664 are selected based on suggestions by the `hc_suggest_topvar()` function in the satellite markdown
...
## 3.1 Module detection
  ...
  NOTE 2: Instead of clustering your network here, you can alternatively import a clustering model via `hc_import_clusters()` (see satellite markdown for details).
...
#   plot_per_comparison = TRUE
# )
# For a condition-specific discovery view, set `consistent_terms` to `FALSE`.
