...
enriched_keys <- function(file = FALSE, path, enrichment_keys = NULL,
  ...
  if (!file) {
    if (is.null(enrichment_keys)) {
      warning("file is set to FALSE but no enrichment keys were provided", call. = FALSE)
      return(NULL)
    }
    enriched_list <- list()
    ...
    for (c in clusters) {
      ...
      if (db == "GO") {
        ...
        if (global_settings$organism %in% c("human", "Human")) {
          entrez <- clusterProfiler::bitr(genes, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = "org.Hs.eg.db", drop = FALSE)$ENTREZID
        ...
        if (global_settings$organism %in% c("mouse", "Mouse")) {
          entrez <- clusterProfiler::bitr(genes, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = "org.Mm.eg.db", drop = FALSE)$ENTREZID
          enrich <- clusterProfiler::enrichKEGG(entrez,
            organism = "mmu",
            pvalueCutoff = 0.05
          )
        ...
        warning("invalid database", call. = FALSE)
        return(NULL)
      }
      ...
      hits <- 0
      top_10 <- enrich@result[order(enrich@result$Count, decreasing = TRUE), ]
      top_10 <- top_10[base::seq_len(base::min(20L, base::nrow(top_10))), ]
      for (type in enrichment_keys) {
        tmp <- dplyr::filter(top_10, grepl(type, top_10$Description, ignore.case = TRUE) == TRUE) %>%
          dplyr::pull(., "geneID") %>%
          paste0(., collapse = "/") %>%
          base::strsplit(., split = "/") %>%
          unlist(.) %>%
      ...
      tmp <- data.frame(matrix(unlist(cell_enrich$counts), ncol = length(cell_enrich$counts), byrow = TRUE) %>% t(), stringsAsFactors = FALSE) %>%
      ...
      for (n in names(cell_enrich$genes)) {
        base::writeLines(base::c("", n, "", cell_enrich$genes[[n]]))
      }
  ...
  if ("enriched_per_cluster" %in% names(hcobject[["satellite_outputs"]])) {
    .hc_set_bridge_hcobject_slot(c("satellite_outputs", "enriched_per_cluster2"), output)
  } else {
