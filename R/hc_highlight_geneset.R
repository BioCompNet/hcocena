...
highlight_geneset <- function(gene_set, name = NULL, col = "black", label_offset = 3, plot = TRUE, save = TRUE) {
  ...
  if (base::length(igraph::V(g)$name[!igraph::V(g)$name %in% gtc$name]) > 0) {
    gtc <- base::rbind(gtc, base::data.frame(
      name = igraph::V(g)$name[!igraph::V(g)$name %in% gtc$name],
      color = "white"
    ))
    gtc$name <- base::as.character(gtc$name)
    gtc$color <- base::as.character(gtc$color)
  ...
  if (is.null(hcobject[["integrated_output"]][["cluster_calc"]][["layout"]])) {
    ...
    l <- .hc_with_seed(123L, igraph::layout.fruchterman.reingold(g))
  ...
  new_genes_df <- base::data.frame(
    name = base::paste0("label_", base::seq_along(gene_set)),
  ...
  new_position_l <- base::matrix(
    base::cbind(
      base::rep(
        base::ceiling(base::min(l[, 1])) - label_offset,
        base::length(left_up) + base::length(left_down)
      ),
      base::seq(
        from = base::ceiling(base::max(l[, 2])),
  ...
  new_position_r <- base::matrix(
    base::cbind(
      base::rep(
        base::ceiling(base::max(l[, 1])) + label_offset,
  ...
  name_to_id <- base::data.frame(name = igraph::V(network3)$name, id = base::seq_along(igraph::V(network3)$name))
