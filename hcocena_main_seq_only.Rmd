...
## OPTIONAL: Correlation cutoff tuning (`hc_tune_cutoff`)
  ...
  This is the recommended cutoff tuner and uses a staged decision strategy:
  1. `strict`: strict criteria
  2. `relaxed`: relaxed criteria
  3. `best_available`: best available SFT fit above a minimum threshold
  ...
  - `tier1_sft_min`, `tier1_mean_k_min`: make the `strict` rule harder/easier.
  - `tier2_sft_min`, `tier2_mean_k_min`: quality thresholds for the `relaxed` rule.
  - `fallback_sft_min`: minimum acceptable fit in `best_available`.
  ...
  - `fallback_respect_base_filters`: if `TRUE`, `best_available` also respects slope/components/node filters.
  - `apply`: `FALSE` (recommend only) or `TRUE` (write cutoffs into object).

  Output locations:
  - `hc@satellite$cutoff_tuning$comparison_with_simple`: selected (`strict`/`relaxed`/`best_available`) vs `simple`
...
```
  ...
  **Choosing the cut-offs**
  ...
  If `hc_tune_cutoff()` was run before, `selected`, `strict`, `relaxed`, `best_available`, and `simple` cutoffs are shown as vertical marker lines.
...
```
  ...
  Running `hc_plot_cluster_heatmap()` writes `Module_Gene_List.xlsx`
  to the output folder (`genes` in column 1, `module` in column 2).
...
```
  ...
  After splitting and re-plotting, `Module_Gene_List.xlsx` is regenerated and
  includes split module labels (e.g. `M1.1`, `M1.2`).
...
hc <- hc_upstream_inference(
  ...
  # custom_pathway_gmt = c(MyPathways = "my_custom_pathways.gmt"),
  heatmap_side = "left",
  plot = TRUE,
  save_pdf = TRUE,
...
saveRDS(hc, file = "hc_after_main.rds")
cat("Saved hCoCena object to: hc_after_main.rds\n")
