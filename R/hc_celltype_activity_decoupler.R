...
#' `hc_plot_cluster_heatmap(include_dynamic_enrichment_slots = TRUE)`.
#'
...
.hc_celltype_activity_decoupler_driver <- function(
  ...
  resolve_group_labels <- function(anno_df) {
    ...
    if (base::length(voi) == 0) {
      return(base::as.character(anno_df[[1]]))
    }
    vals <- anno_df[, voi, drop = FALSE]
    if (base::ncol(vals) == 1) {
      return(base::as.character(vals[[1]]))
    }
  ...
  prepare_group_mean_matrix <- function(set_name) {
    ...
    if (base::is.null(expr) || base::is.null(anno)) {
      return(NULL)
    }
    expr <- expr %>% base::as.matrix()
    if (base::nrow(expr) == 0 || base::ncol(expr) == 0) {
      return(NULL)
    }
    mode(expr) <- "numeric"
  ...
  if (identical(activity_input, "gfc")) {
    ...
    for (z in base::seq_len(base::length(hcobject[["layer_specific_outputs"]]))) {
      ...
      for (i in base::seq_len(base::nrow(fc_def))) {
        num <- base::as.character(fc_def$numerator[i])
        den <- base::as.character(fc_def$denominator[i])
        cmp <- base::as.character(fc_def$comparison[i])
        if (!(num %in% base::colnames(sm) && den %in% base::colnames(sm))) next
        comparison_found[[cmp]] <- TRUE
        fc_vals <- .hc_log2_safely(
          (.hc_as_numeric_safely(sm[ov, num]) + pseudo) / (.hc_as_numeric_safely(sm[ov, den]) + pseudo)
        )
  ...
  combine_non_empty <- function(lst, with_condition = FALSE) {
    ...
    if (base::length(lst) == 0) {
      return(empty_df(with_condition = with_condition))
    }
  ...
  for (db_nm in db_names) {
    ...
    for (cl in clusters) {
      ...
      if (base::nrow(agg) > 0) {
        agg$resource <- "CellType"
        agg$database <- db_nm
        agg$cluster <- cl
        agg$module_label <- if (cl %in% base::names(module_label_map)) base::as.character(module_label_map[[cl]]) else cl
        agg$cell_type <- base::as.character(agg$term)
        agg$n_genes <- base::length(base::unique(genes))
        agg$n_targets <- base::length(base::unique(net_mod$target))
        ...
        if (base::nrow(sig) > 0) {
          ...
          sel$rank <- base::seq_len(base::nrow(sel))
      ...
      if (base::nrow(aggc) > 0) {
        aggc$resource <- "CellType"
        aggc$database <- db_nm
        aggc$cluster <- cl
        aggc$module_label <- if (cl %in% base::names(module_label_map)) base::as.character(module_label_map[[cl]]) else cl
        aggc$cell_type <- base::as.character(aggc$term)
        aggc$n_genes <- base::length(base::unique(genes))
        aggc$n_targets <- base::length(base::unique(net_mod$target))
  ...
  for (db_nm in db_names) {
    ...
    sel_db$count <- .hc_as_numeric_safely(sel_db$abs_score)
    sel_db$count[!base::is.finite(sel_db$count)] <- 0
