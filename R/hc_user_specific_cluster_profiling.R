...
#'   The enrichment results will be visualized as stacked bar plot annotations on
#'   the module heatmap when rerunning `hc_plot_cluster_heatmap()`.
  ...
  if (!from_file) {
    ...
    for (c in clusters) {
      ...
      for (type in enrichment_keys) {
        tmp <- dplyr::filter(top_terms, base::grepl(type, top_terms$Description, ignore.case = TRUE) == TRUE) %>%
          dplyr::pull(., "geneID") %>%
    ...
    for (c in clusters) {
      ...
      tmp <- base::data.frame(base::matrix(base::unlist(cell_enrich$counts), ncol = base::length(cell_enrich$counts), byrow = TRUE) %>% base::t(), stringsAsFactors = FALSE) %>%
        base::cbind(., base::names(cell_enrich$counts)) %>%
        base::cbind(., base::rep(c, base::length(cell_enrich$counts)))
