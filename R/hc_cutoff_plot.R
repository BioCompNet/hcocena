...
plot_cutoffs <- function(interactive = TRUE,
                         hline = list(
                           "R.squared" = NULL,
                           "no_edges" = NULL,
                           "no_nodes" = NULL,
  ...
  .hc_get_cutoff_tuning_marker <- function(layer_id, layer_index) {
    ...
    .extract_from_summary <- function(summary_df, flag_col) {
      ...
      cuts <- .hc_as_numeric_safely(summary_df$cutoff)
    ...
    if (!base::is.null(rec)) {
      rec_num <- .hc_as_numeric_safely(rec)
  ...
  for (x in base::seq_along(hcobject[["layers"]])) {
    ...
    if (interactive == TRUE) {
      p <- plot_cutoffs_internal_interactive(
        cutoff_stats = cutoff_df,
        x = x,
        tuning_marker = tuning_marker
      )
    } else {
      p <- plot_cutoffs_internal_static(
        cutoff_stats = cutoff_df,
        hline = hline,
        x = x,
        tuning_marker = tuning_marker
    ...
    .hc_display_object(p)
    .hc_set_bridge_hcobject_slot(c("layer_specific_outputs", base::paste0("set", x), "cutoff_plot"), p)
  }
}

.hc_plot_cutoffs_driver <- plot_cutoffs
...
plot_cutoffs <- function(interactive = TRUE,
  ...
  .hc_alias_warning("plot_cutoffs")
  out <- .hc_run_modern_bridge_capture(
    .hc_plot_cutoffs_impl,
