...
.hc_split_modules_driver <- function(modules,
  ...
  if (is.null(min_submodule_size)) {
    min_submodule_size <- .hc_first_numeric_value(
  ...
  if (!is.null(resolution_grid)) {
    ...
    if (isTRUE(verbose)) {
      ...
      .hc_display_object(preview$by_resolution, row.names = FALSE)
    }
...
.hc_unsplit_modules_driver <- function(which = c("last", "all"), verbose = TRUE) {
  ...
  .hc_set_bridge_hcobject_slot(c("integrated_output", "cluster_calc", "cluster_information"), before[["cluster_information"]])
  if (!is.null(before[["module_label_map"]])) {
    .hc_set_bridge_hcobject_slot(c("integrated_output", "cluster_calc", "module_label_map"), before[["module_label_map"]])
  } else {
    .hc_set_bridge_hcobject_slot(c("integrated_output", "cluster_calc", "module_label_map"), NULL)
  }
  .hc_set_bridge_hcobject_slot(c("integrated_output", "cluster_calc", "heatmap_cluster"), NULL)
...
.hc_split_membership <- function(graph_obj,
  ...
  get_membership_single <- function(algo) {
    if (algo == "cluster_leiden") {
      ...
      mem <- .hc_as_integer_safely(part$membership) + 1L
    ...
    if (is.list(mem) && !is.numeric(mem)) {
      ...
      for (ii in seq_along(mem)) {
        idx <- .hc_as_integer_safely(mem[[ii]])
