...
rank_variance <- function(expr) {
  ...
  var.df <- var.df[base::order(var.df[["variance"]], decreasing = TRUE), ]
  var.df[["rank"]] <- base::seq_len(base::nrow(var.df))
  return(var.df)
}
...
plot_inflections <- function(var.df, i, setname = "my data") {
  ...
  message(
    setname,
    ": Inflection points at the following #genes: ",
    base::paste(base::ceiling(xl[infl]), collapse = ", ")
  )
}
...
.hc_suggest_topvar_driver <- suggest_topvar

suggest_topvar <- function() {
  .hc_run_alias_via_modern("suggest_topvar", hc_suggest_topvar)
}
